...
const AddExpenseModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (MOCK_GROUPS.length > 0) {
        setFormData((prev) => ({ ...prev, group: MOCK_GROUPS[0]._id }));
        loadGroupMembers(MOCK_GROUPS[0]._id);
      }
    }
  }, [isOpen]);
  ...
  const loadGroupMembers = (groupId) => {
    ...
    if (group) {
      ...
      if (currentUserId && members.find((m) => m._id === currentUserId)) {
  ...
  const handleSubmit = (e) => {
    ...
    const splitAmount = parseFloat(formData.amount) / selectedUsers.length;
    ...
    const expenseData = {
      ...
      groupName: groups.find((g) => g._id === formData.group)?.name || "",
      paidBy: formData.paidBy,
      paidByName:
        MOCK_USERS.find((u) => u._id === formData.paidBy)?.name || "Unknown",
      paidByAvatar:
        MOCK_USERS.find((u) => u._id === formData.paidBy)?.avatar || "🧑",
      date: "Today",
      members: selectedUsers.length,
      icon: getCategoryIcon(formData.category),
      splitDetails: selectedUsers.map((user) => ({
        name: user.name,
        email: user.email,
        avatar: user.avatar,
        amount: splitAmount,
        isPaidBy: user._id === formData.paidBy,
      })),
    };
